...
export const predefinedStyles: PredefinedStyle[] = [
  ...
  {
    name: '社群文風',
    template: '請以吸引人的社群媒體風格撰寫。開頭使用相關的 hashtag 或震撼性問句來抓住注意力。內容要融入數據支撐的論點，但用輕鬆對話的語調表達。多使用「你知道嗎？」、「說真的」、「重點來了」等口語化開場。段落要短而有力，適合手機閱讀。加入個人經驗分享和情感共鳴。結尾要有自然的行動呼籲，不要過於生硬。整體風格要像朋友在社群平台分享有價值的見解，既專業又親近。',
    category: '特定文體與領域風格'
  }
];
